...
- We do **not** use "clone ChatGPT memory" framing anywhere in this repo.
...
Evidence sources, all already persisted:

- `visible_turns` written via `/v1/turns`;
- explicit `conclusions` via `/v1/conclusions`;
- `checkpoints` via `/v1/checkpoints`;
- imported local Codex memories via `/v1/sync/local-codex-memory`;
- existing active `memory_records` and their `last_used_at`.
...
Proposed record metadata (additive, no migration — `metadata` is already a free
JSON `Value` on `MemoryRecord`):
...
seeded under `tests/fixtures/dreaming/`). Eval questions:
...
- Did recall improve for a future prompt?
...
| --- | --- | --- |
| **0 — research** (this PR) | `docs/dreamer-loop-research.md`, `docs/dreamer-loop-design.md`, seeded eval fixtures | none |
| **1 — preview skeleton** | `src/dream.rs`, `codex-memoryd dream --preview`, source gathering + candidate reporting, no-write + policy-rejection tests | none |
| **2 — apply mode** | idempotent writes, supersession/archive, provenance metadata, conflict/staleness rules, conformance tests | gated |
| **3 — daemon/scheduler** | bounded batch size, rate/cost controls, retry/error state, metrics/logging, status surface | gated |
| **4 — MCP/App bridge** | MCP tool surface, remote auth model, optional ChatGPT app wrapper | gated |
